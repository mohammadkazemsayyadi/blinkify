...
// INPUT VALIDATION
// ==========================================
// Instantly convert negative numbers to absolute values while the user types
[intervalInput, durationInput].forEach(input => {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  //Set the volume slider to the saved value
  if (stored.volume !== undefined) {
    document.getElementById('volumeSlider').value = stored.volume;
  }
...
async function triggerStart() {
  // Enforce absolute values AND a minimum of 1 to prevent 0-minute crash loops
  const interval = Math.max(1, Math.abs(parseInt(intervalInput.value, 10)) || 10);
  const duration = Math.max(1, Math.abs(parseInt(durationInput.value, 10)) || 60);
...
if (volumeSlider) {
  volumeSlider.addEventListener('input', async (e) => {
    ...
    // Save to local storage directly just in case
    await chrome.storage.local.set({ volume: newVolume });
    
    // Send to background script to update settings state
    chrome.runtime.sendMessage({ 
      type: 'updateSettings', 
      volume: newVolume 
    });
    
    // Send directly to the offscreen document for live, real-time volume adjustment
    chrome.runtime.sendMessage({ 
      action: 'setVolume', 
      volume: newVolume 
    });
